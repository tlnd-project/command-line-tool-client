...
class LocalServerClusterManagment:
  ...
  def cluster_has_servers(self, virtual_server_name: str) -> bool:
    return len(self.clusters_map[virtual_server_name].get('servers', [])) > 0
  ...
  def get_virtual_servers(self) -> list:
    return [val for key,val in self.clusters_map.items()]
